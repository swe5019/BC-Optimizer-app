...
tab1, tab2, tab3 = st.tabs(["🏌️ Best Ball Rounds", "🎯 Singles Round", "📈 Pairing Rankings"])
...
with tab2:
    ...
    for i, (who_first, p1, p2) in enumerate(st.session_state.singles_matches, 1):
        ...
        st.write(f"**Match {i}:** {p1[0]} ({p1[1]}) vs {p2[0]} ({p2[1]}) → {abs(diff):.2f} ({higher_gets} gets strokes)")
    st.markdown(f"### 🧮 Total Stroke Advantage: 🟢 Atown = {total_a:.2f} | 🟠 Pittsburgh = {total_b:.2f}")
